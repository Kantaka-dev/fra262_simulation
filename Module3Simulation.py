...
class InputBox:
    ...
    def draw(self, mode=1):
        ...
        if mode == 0:
            ...
            screen.blit(FONT(28).render(self.value, True, COLOR['WHITE']), (self.x+80, self.y+10))
            return
        # Box
        elif self.able:
        ...
        # Font
        screen.blit(FONT(28).render(self.value, True, (0,0,0)), (self.x+80, self.y+10))
...
class Simulation:
    def __init__(self, input_box, max_rpm=10):
...
# Run
    ...
    def drawEndEffector(self, mode=0, linkstyle=7):
        link_length = [self.radius*math.cos(DtoR(self.theta[0])), self.radius*math.sin(DtoR(self.theta[0]))]
        # Mode:1
        if mode == 1:
            ...
            self.end_effector = [self.center[0] + link_length[0], self.center[1] + link_length[1]]
        ...
        for i in range(1, linkstyle+1):
            pg.draw.circle(screen, COLOR['BLACK'], (
                int(self.end_effector[0] -(i/linkstyle)* link_length[0]),
                int(self.end_effector[1] -(i/linkstyle)* link_length[1])
            ), 20)
        # draw center
